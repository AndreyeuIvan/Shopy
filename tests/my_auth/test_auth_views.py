...
class TestLoginView(BaseUserTest):
    ...
    def test_login_password_failed(self):
        response = self.custom_authentication(
            username="TestUsername", password=self.password
        )
        self.assertEqual(
            response.status_code,
    ...
    def test_login_two_users_one_by_one(self):
        ...
        response_user_login = self.custom_authentication(
            password=self.password, username=self.user.username
        )
        self.assertEqual(response_user_login.status_code, status.HTTP_202_ACCEPTED)
        self.assertEqual(response_user_login.data["id"], self.user.id)
        self.assertEqual(response_user_login.data["username"], self.user.username)
        self.assertEqual(
        ...
        response_other_user_login = self.custom_authentication(
            password=self.password, username=self.other_user.username
        )
        self.assertEqual(
            response_other_user_login.status_code, status.HTTP_202_ACCEPTED
        )
        self.assertEqual(
            response_other_user_login.data["username"], self.other_user.username
        )
        self.assertEqual(
            response_other_user_login.data["username"], self.other_user.username
        )
...
class TestRegisterView(BaseUserTest):
    ...
    def test_sign_up_and_user_is_still_login(self):
        sign_up_response = self.custom_sign_up(
            username="Test", password=self.password, email="12@icloud.com"
        )
        self.assertEqual(
        ...
        self.assertEqual(sign_up_response.wsgi_request.user.id, session_user_id)
    ...
    def test_same_username_register(self):
        ...
        self.assertEqual(
            sign_up_user.status_code,
            status.HTTP_201_CREATED,
            f"Expected 201, received {sign_up_user.status_code}",
        )
        sign_up_other_user = self.custom_sign_up(
            username="Test", password=self.password, email="10@icloud.com"
        )

        self.assertEqual(
            sign_up_other_user.status_code,
            status.HTTP_400_BAD_REQUEST,
    ...
    def test_wrong_email_format(self):
        sign_up_other_user = self.custom_sign_up(
            username="Test1", password=self.password, email="11@icloudcom"
        )
        self.assertEqual(
            sign_up_other_user.status_code,
            status.HTTP_400_BAD_REQUEST,
        ...
        self.assertEqual(
            "".join(sign_up_other_user.data["email"]), "Enter a valid email address."
        )

    def test_not_ununique_email(self):
        sign_up_user = self.custom_sign_up(
            username="Test", password=self.password, email="11@icloud.com"
        )
        self.assertEqual(
            sign_up_user.status_code,
            status.HTTP_201_CREATED,
        ...
        sign_up_other_user = self.custom_sign_up(
            username="Test1", password=self.password, email="11@icloud.com"
        )
